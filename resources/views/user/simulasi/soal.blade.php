...
<div class="row">
    ...
    <div class="col-md-12 mb-3" v-if="isMounted">
        <a href="{{ route('user.simulasi.ujian.finish', ['id' => $simulasi->id, 'idAttempt' => $attempt->id])}}" class="btn btn-success btn-block selesai-ujian">SELESAIKAN SEKARANG</a>
    </div>
    <div class="col-md-12 mb-3" v-if="isMounted">
...
var linkRequestSoal = "{{ route('user.ujian.attempt.request.soal', ':id') }}";
...
var app = new Vue({
    ...
    methods: {
        ...
        sendJawaban: function(){
            var self = this;
            var url = "{{ route('user.ujian.attempt.sendJawaban') }}";
            axios({
                method: 'post',
                url: url,
                data: {
