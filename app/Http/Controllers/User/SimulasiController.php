...
class SimulasiController extends Controller
{
    ...
    public function register($id) {
        ...
        $isRegistered = SimulasiPeserta::where('id_simulasi', $simulasi->id)
                                        ...
                                        ->first();
        if($isRegistered)
            return redirect()->route('user.simulasi.open', $simulasi->id);

        $universitas = Universitas::all();
        return view('user.simulasi.passgrade')->with([
            'simulasi' => $simulasi,
    ...
    public function registerPost(Request $input, $id) {
        ...
        if($peserta) return redirect()->route('user.simulasi.open', $simulasi->id)->with("success", "Anda sudah mendaftar pada Simulasi " . $simulasi->judul);
        ...
        if($peserta->save()) {
            ...
            if($passingGrade->save()) {
                return redirect()->route('user.simulasi.open', $simulasi->id)->with("success", "Berhasil mendaftar pada Simulasi " . $simulasi->judul);
            }
            else {
                $peserta->delete();
    ...
    public function open($id) {
        ...
        if($peserta) {
            if($peserta->mode_simulasi == "offline") {
                return view('user.simulasi.open')->with([
                    'simulasi' => $simulasi,
        ...
        return redirect()->route('user.simulasi');
    }
    ...
    public function kartuUjian($id) {
        ...
        // return view('user.simulasi.kartuujian')->with([
        //     'peserta' => $peserta
        // ]);
    }
    ...
    public function passGrade($id) {
        ...
        if(!$peserta)
            return redirect()->route('user.simulasi');
        $universitas = Universitas::all();
        return view('user.simulasi.updatepassgrade')->with([
            'simulasi' => $simulasi,
            'universitas' => $universitas,
            'peserta' => $peserta
        ]);
    ...
    public function passGradePost(Request $input, $id) {
        ...
        $passingGrade->pilihan_3 = $input->jurusan_3;
        if($passingGrade->save()) {
            return redirect()->route('user.simulasi.open', $simulasi->id)->with("success", "Berhasil menyimpan perubahan pilihan Passing Grade");
        }
        else {
            return back()->with('danger', "Gagal Menyimpan Passing Grade");
        }
    ...
    public function attempt($id, $idUjian) {
        ...
        if(($attemptOnGoing != null) || (strtotime($peserta->jadwalOnline->tanggal) != strtotime(date("Y-m-d")))) {
            return redirect()->route('user.simulasi.ujian.open', [
                'id' => $simulasi->id,
    ...
    public function openUjian($id, $idAttempt) {
        ...
        $attempt = Attempt::where('id_user', Auth::id())
                        ...
                        ->where('end_attempt', '>=', date('Y-m-d H:i:s'))
                        ->first();
        if($attempt == null) return redirect()->route('user.simulasi.open', $simulasi->id);
        $soal = Ujian::findOrFail($attempt->id_ujian);
        return view('user.simulasi.soal')->with([
            'soal' => $soal,
            'attempt' => $attempt,
            'simulasi' => $simulasi
        ]);
    ...
    public function finish($id, $idAttempt) {
        ...
        $attempt = Attempt::where('id_user', Auth::id())
                        ...
                        ->where('end_attempt', '>=', date('Y-m-d H:i:s'))
                        ->first();
        if($attempt == null) return redirect()->route('user.ujian.soal');
        $attempt->end_attempt = date("Y-m-d H:i:s");
        $attempt->save();
        return view('user.simulasi.finish')->with([
            'simulasi' => $simulasi,
            'attempt' => $attempt
        ]);
    }
    ...
    public function lihatHasil($id) {
        ...
        return view('user.simulasi.lihathasilsementara')->with([
            'simulasi' => $simulasi,
            'peserta' => $peserta,
            'koreksi' => $koreksi
        ]);
